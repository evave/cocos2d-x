...
public:
    enum {
        ...
        UsageTarget = 1 << 3,
        Usage2D = 1 << 4,
        UsageFramebuffer = 1 << 5
    };

    AndroidGraphicBuffer(uint32_t width, uint32_t height, uint32_t usage, CCTexture2DPixelFormat format);
    virtual ~AndroidGraphicBuffer();

    int Lock(uint32_t usage, unsigned char **bits);
    int Lock(uint32_t usage, const CCRect& rect, unsigned char **bits);
    int Unlock();
    bool Reallocate(uint32_t aWidth, uint32_t aHeight, CCTexture2DPixelFormat aFormat);
    ...
    static bool IsBlacklisted(std::string &deviceModel);
...
private:
    ...
    uint32_t GetAndroidFormat(CCTexture2DPixelFormat aFormat);
